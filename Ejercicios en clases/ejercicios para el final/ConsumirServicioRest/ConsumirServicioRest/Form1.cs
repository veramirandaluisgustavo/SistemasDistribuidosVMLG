...
namespace ConsumirServicioRest
{
    public partial class Form1 : Form
    {
        ...
        {
            string apiUrl = this.url + "cliente";
            ...
            using (HttpClient client = new HttpClient())
            {
...

                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        // Deserializar la respuesta JSON a un objeto C#
                        var jsonObject = JsonConvert.DeserializeObject<RootObject>(responseBody);

                        // Filtrar usuarios mayores de 25 años
                        var usuariosMayoresDe25 = jsonObject.cuentas.FindAll(u => u.numero_cuenta == boxId.Text);

                        // Mostrar los usuarios filtrados
                        Console.WriteLine("Usuarios mayores de 25 años:");
                        foreach (var usuario in usuariosMayoresDe25)
                        {
                            Console.WriteLine($"ID: {usuario.id}, codigo: {usuario.codigo_cliente}");
                            MessageBox.Show("el saldo de la cuenta es: " + $"SALDO: {usuario.saldo}");
                        }
